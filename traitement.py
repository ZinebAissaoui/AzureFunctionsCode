...
def traitement_prescrapping(df):
    df=df[['First Name','Last Name', 'URL','Company','Position','Position Stratégique', 'Domaine','Fav','Owner','mail_ver']]
    df=mail_treatement(df)
    for index, row in df.iterrows():
        if (row['mail_ver'] != '') & pd.notna(row['mail_ver'] )& (row['mail_ver']!='nan'):
            if ';' in str(row['mail_ver'] ):
                df.at[index, 'mail1'] = str(row['mail_ver']).split(';')[0]
            else :
                df.at[index, 'mail1'] = str(row['mail_ver'])
        else :
            df.at[index, 'mail1'] = ''
...
def traitement_postscrapping(df,dfcities):
    ...
    df=df[['First Name','Last Name', 'URL','Company','Position','Position Stratégique', 'Domaine','Fav','Owner','mail_ver','mail1','Localisation']]
