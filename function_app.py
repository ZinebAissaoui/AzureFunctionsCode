...
from convention import convention_,remove_empty_values
...
def blob_trigger_convention(myblob0: func.InputStream):
    ...
    if blob_client_total.exists():
        ...
        conventions=convention_(processed_data,convention)
        conventions=remove_empty_values(conventions)
        merged_json = json.dumps(conventions, sort_keys=True)
        ...
        conventions=convention_(processed_data, conventions={})
        conventions= remove_empty_values(conventions)
        json_data = json.dumps(conventions , sort_keys=True)
...
def blob_trigger_generation_email(myblob5: func.InputStream):
    ...
    logging.info("File prescrapping.csv processed and uploaded successfully in Outputprescrap.")
...
def blob_triggertestscrap(myblob6: func.InputStream):
    ...
    if blob_client_loading.exists():
        ...
        logging.info("file loading non disponible")
    ...
    logging.info(f"upload  {file_name_} in {container_name_}")
